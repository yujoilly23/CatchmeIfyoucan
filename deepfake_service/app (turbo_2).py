...
위 문장의 사실 관계를 분석하여, 사실과 다르거나 왜곡된 정보, 잘못된 오류가 있는지 분석해 줘.
별도의 복잡한 항목 분류 없이 '발견된 오류' 내용만 구체적인 근거와 함께 명확하게 설명해 줘.
(내용이 사실과 일치하여 왜곡이나 오류가 없다면 '발견된 오류 없음 (사실과 일치함)'이라고만 답변해 줘.)"""}
    ]
    ...
    try:
        ...
        return f"로컬 모델 오류 분석 처리 중 오류: {str(e)}"
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown("# 🕵️‍♂️ 딥페이크 탐지 & AI 발화 오류 분석 시스템")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            txt_corrected = gr.Textbox(label="❌ 발견된 오류 분석 결과", lines=6)
