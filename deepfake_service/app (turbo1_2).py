...
def fact_check_and_correct(transcribed_text):
    ...
    messages = [
        {"role": "system", "content": "너는 신뢰성 높은 한국어 팩트체커이자 발화 검증 전문가야."},
        {"role": "user", "content": f"""다음은 영상 음성 인식으로 추출된 문장입니다:
"{transcribed_text}"

위 내용을 분석하여 아래 두 가지 항목 형식으로 한국어로 답변해 줘:
1. 🔍 사실 관계 검증: 사실과 다르거나 왜곡된 주장이 있는지 분석 (문제가 없다면 '사실과 부합함' 명시)
2. ❌ 발견된 오류: 어떤 점이 사실과 다르고 왜 잘못되었는지 간단명료하게 설명 (오류가 없다면 '없음' 명시)"""}
    ]
    ...
    try:
        ...
        return f"로컬 모델 팩트체크 처리 중 오류: {str(e)}"
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    gr.Markdown("영상 속 **얼굴 합성(딥페이크)** 여부를 판정하고, **음성 인식 텍스트의 사실 관계 및 오류를 분석**합니다.")
    ...
    with gr.Row():
        with gr.Column(scale=1):
            txt_raw = gr.Textbox(label="🎙️ 음성 인식 원문 (Faster-Whisper STT)", lines=6)
        with gr.Column(scale=1):
            txt_corrected = gr.Textbox(label="💡 AI 사실 관계 검증 및 오류 분석 결과", lines=6)
