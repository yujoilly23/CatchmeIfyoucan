...
# 🎨 1. 커스텀 테마 설정 (전체 배경색 #CFAE78 적용)
custom_theme = gr.themes.Soft(
    primary_hue="amber",
    neutral_hue="stone"
).set(
    # --- [🔥 전체 바탕 배경색 변경] ---
    background_fill_primary="#CFAE78",  # 요청하신 배경색 적용
    
    # --- [패널 배경색 변경] ---
    block_background_fill="#F8F5F0",  # 박스 안쪽은 글씨가 잘 보이도록 부드러운 크림색 유지
    input_background_fill="#FFFFFF",  # 입력창(텍스트박스 등)은 하얀색 유지
    
    # --- [메인 버튼 색상 변경] ---
    button_primary_background_fill="#5C3A21",        
    button_primary_background_fill_hover="#7A4E2C",  
    button_primary_text_color="#FFFFFF",             
    
    # --- [테두리 색상 변경] ---
    block_border_color="#B59560",     # 배경색(#CFAE78)과 어울리도록 테두리 톤 다운
    block_border_width="1px"
)

# 🎨 2. 확실한 배경색 적용을 위한 CSS 추가
custom_css = """
/* Gradio 기본 여백까지 완벽하게 #CFAE78 색상으로 덮어줍니다 */
body, .gradio-container {
    background-color: #CFAE78 !important;
}
"""

# 🚀 3. 테마와 CSS를 적용하여 UI 시작
    ...
    with gr.Row():
        with gr.Column(scale=1):
            with gr.Group():
                gr.Markdown("### 발화 내용")
                txt_raw = gr.Textbox(
                    show_label=False,  # 이 옵션이 "Textbox" 글자를 완전히 없애줍니다!
                    lines=6, 
                    interactive=False, 
                    placeholder="음성에서 추출된 텍스트가 여기에 기록됩니다."
)
        with gr.Column(scale=1):
            with gr.Group():
                gr.Markdown("### 팩트체크!")
                txt_corrected = gr.Textbox(
                    show_label=False,  # 여기도 추가해줍니다!
                    lines=6, 
                    interactive=False, 
                    placeholder="발화 내용의 진위 여부 및 오류 분석 결과가 여기에 기록됩니다."
)
